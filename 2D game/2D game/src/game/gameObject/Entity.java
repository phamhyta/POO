...
public abstract class Entity extends GameObject{
    ...
    protected boolean isInCircle (Vector2f center, double r) {
        if (Math.pow(this.getPos().x-center.x,2) +Math.pow(this.getPos().y-center.y,2)< r*r) {
            return true;
        }
        return false;
    }
    protected boolean isInCirclePath (Vector2f center, double r) {
        if (Math.pow(this.getPos().x-center.x,2) +Math.pow(this.getPos().y-center.y,2)  > (r-size)*(r -size)
                && Math.pow(this.getPos().x-center.x,2) +Math.pow(this.getPos().y-center.y,2) < (r+ size)*(r +size)) {
            return true;
        }
        return false;
    }
