...
public class ShopState extends GameState {
    ...
    private int shop = 0;
    private int buy = 0, sell = 0;
    private int back = 0;
    private int slotCol = 0;
    private int slotRow = 0;
    private ArrayList<GameObject> items, inv;
    ...
    public void drawIntro(Graphics2D g2) {
        ...
        drawSubWindow(g2, xBuy - 40, yBuy + 268, 200, height / 2);
        g2.drawString("Buy", xBuy - 40 + 64, yBuy + 48 + 268);
        if (commandNum == 0) {
            g2.drawString(">", xBuy - 40 + 32, yBuy + 48 + 268);
        }
        g2.drawString("Sell", xBuy - 40 + 64, yBuy + 48 * 2 + 268);
        if (commandNum == 1) {
            g2.drawString(">", xBuy - 40 + 32, yBuy + 48 * 2 + 268);
        }
        g2.drawString("Leave", xBuy - 40 + 64, yBuy + 48 * 3 + 268);
        if (commandNum == 2) {
            g2.drawString(">", xBuy - 40 + 32, yBuy + 48 * 3 + 268);
        }
    }
    ...
    public void sell(GameObject go) {
        if (inv.size() > 0) {
            player.removeMaterial(go);
            player.setCoin(player.getCoin() + go.getCoin() * 2 / 3);
        }
    }
    ...
    public void drawAttributes(Graphics2D g2, int slotX, int slotY) {
        ...
        if (i < items.size() && i >= 0) {
            ...
            if (items.get(i).getHP() != 0) {
                g2.drawString("+" + String.valueOf(items.get(i).getHP()) + " HP", x + width * 3 / 2 + size / 2,
                        y + 3 * size / 2 + cnt * size);
                cnt++;
            }
            if (items.get(i).getMP() != 0) {
                g2.drawString("+" + String.valueOf(items.get(i).getMP()) + " MP", x + width * 3 / 2 + size / 2,
                        y + 3 * size / 2 + cnt * size);
                cnt++;
            }
            if (items.get(i).getAttackValue() != 0) {
                g2.drawString("+" + String.valueOf(items.get(i).getAttackValue()) + " attack",
                        x + width * 3 / 2 + size / 2, y + 3 * size / 2 + cnt * size);
                cnt++;
            }
            if (items.get(i).getDefense() != 0) {
                g2.drawString("+" + String.valueOf(items.get(i).getDefense()) + " defense",
                        x + width * 3 / 2 + size / 2, y + 3 * size / 2 + cnt * size);
                cnt++;
            }
            if (items.get(i).getSpeed() != 0) {
                g2.drawString("+" + String.valueOf(items.get(i).getSpeed()) + " Speed", x + width * 3 / 2 + size / 2,
                        y + 3 * size / 2 + cnt * size);
                cnt++;
            }
            if (items.get(i).getCoin() != 0) {
                g2.drawString("Gia: " + String.valueOf(items.get(i).getCoin()) + " Coins", x + width * 3 / 2 + size / 2,
                        y + 3 * size / 2 + cnt * size);
                cnt++;
            }
        }
    ...
    public void SellAttributes(Graphics2D g2, int slotX, int slotY) {
        int i = slotX + slotY * 10;
        if (i < inv.size() && i >= 0) {
            g2.setFont(new Font("NewellsHand", Font.PLAIN, size * 2 / 3));
            inv.get(i);
            g2.drawString(inv.get(i).getName(), x + width * 3 / 2 + size / 2, y + 3 * size / 2);
            int cnt = 1;
            if (inv.get(i).getHP() != 0) {
                g2.drawString("+" + String.valueOf(inv.get(i).getHP()) + " HP", x + width * 3 / 2 + size / 2,
                        y + 3 * size / 2 + cnt * size);
                cnt++;
            }
            if (inv.get(i).getMP() != 0) {
                g2.drawString("+" + String.valueOf(inv.get(i).getMP()) + " MP", x + width * 3 / 2 + size / 2,
                        y + 3 * size / 2 + cnt * size);
                cnt++;
            }
            if (inv.get(i).getAttackValue() != 0) {
                g2.drawString("+" + String.valueOf(inv.get(i).getAttackValue()) + " attack",
                        x + width * 3 / 2 + size / 2, y + 3 * size / 2 + cnt * size);
                cnt++;
            }
            if (inv.get(i).getDefense() != 0) {
                g2.drawString("+" + String.valueOf(inv.get(i).getDefense()) + " defense",
                        x + width * 3 / 2 + size / 2, y + 3 * size / 2 + cnt * size);
                cnt++;
            }
            if (inv.get(i).getSpeed() != 0) {
                g2.drawString("+" + String.valueOf(inv.get(i).getSpeed()) + " Speed", x + width * 3 / 2 + size / 2,
                        y + 3 * size / 2 + cnt * size);
                cnt++;
            }
            if (inv.get(i).getCoin() != 0) {
                g2.drawString("Gia ban: " + String.valueOf(inv.get(i).getCoin() * 2 / 3) + " Coins",
                        x + width * 3 / 2 + size / 2,
                        y + 3 * size / 2 + cnt * size);
                cnt++;
            }
    ...
    public void drawShop(Graphics2D g2, int x, int y, int row, int col) {
        ...
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++) {
                g2.drawImage(image, slotXstart + j * size * 3 / 2, slotYstart + i * size * 3 / 2, size * 3 / 2,
                        size * 3 / 2, null);
            }
        }
            ...
            if (function == 1 && commandNum == 0) {
                ...
                if (function == 1 && commandNum == 0) {
                    for (int i = 0; i < items.size(); i++) {
                        g.drawImage(items.get(i).getObjectRender().getImage(), slotXstart + slotX * size * 3 / 2,
                ...
                if (buy >= 1) {
                    if (inv.size() > 24) {
                        ...
                        g.setColor(Color.BLACK);
                        g.drawString("Your inventory is full! Backspace to back", x + 2 * size,
                                y + height / 2 + size * 3 / 2);
                        if (back == 1) {
                            buy = 0;
                            back = 0;
                        ...
                        if (player.getCoin() < items.get(slotCol + slotRow * 10).getCoin()) {
                            ...
                            drawSubWindow(g, x + 3 * size / 2, y + height / 2, 650, 80);
                            g.setColor(Color.BLACK);
                            g.drawString("Your coin is not enough! Backspace to back", x + 2 * size,
                                    y + height / 2 + size * 3 / 2);
                            if (back == 1) {
                                buy = 0;
                                back = 0;
            ...
            if (function == 1 && commandNum == 1) {
                ...
                if (function == 1 && commandNum == 1) {
                    for (int i = 0; i < inv.size(); i++) {
                        g.drawImage(inv.get(i).getObjectRender().getImage(),
                                slotXstart + slotX * size * 3 / 2,
                                slotYstart + slotY * size * 3 / 2, 48, 48, null);
                        slotX++;
                        if (slotX > 9) {
                            slotY++;
                ...
                SellAttributes(g, slotCol, slotRow);

                if (sell >= 1 && inv.size() > slotCol + slotRow * 10) {
                    if (sell == 1) {
                        sell(inv.get(slotCol + slotRow * 10));
                    }
                    sell++;
                    if (sell < 30) {
                        g.setColor(Color.yellow);
                        drawSubWindow(g, x + 3 * size / 2, y + height / 2, 600, 80);
                        g.setColor(Color.BLACK);
                        g.drawString("Thanks for selling!", x + 2 * size, y + height / 2 + size * 3 / 2);
                    } else
                        sell = 0;
                }
            }
